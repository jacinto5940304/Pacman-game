...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
    ...
    int tmpX = pacman->objData.Coord.x, tmpY = pacman->objData.Coord.y;
    ...
    if (is_wall_block(M, tmpX, tmpY) || is_room_block(M, tmpX, tmpY))
        return false;
...
//    int offset = 0;
    if (game_over) {
        ...
        */
        ...
        int curX = 16*((al_get_timer_count(pman->death_anim_counter)/4)); //pman->death_anim_counter(64Hz);
        if(curX >= (16*12)) return;
        al_draw_scaled_bitmap(pman->die_sprite, curX, 0,
            16, 16,
            drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
            draw_region, draw_region, 0
        );
        ...
        al_draw_scaled_bitmap(pman->move_sprite, condition*16+dir, 0, 16, 16,
            drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
            draw_region, draw_region, 0);
        flag++;
    }
...
void pacman_eatItem(Pacman* pacman, const char Item) {
    ...
    case 'G':
        stop_bgm(PACMAN_MOVESOUND_ID);
        PACMAN_MOVESOUND_ID = play_audio(PACMAN_GHOST_SOUND, effect_volume);
        break;
    case 'C':
        stop_bgm(PACMAN_MOVESOUND_ID);
        play_audio(PACMAN_LUCKY_SOUND, effect_volume);
        break;
    case 'D':
        stop_bgm(PACMAN_MOVESOUND_ID);
        play_audio(PACMAN_ANGRY_SOUND, effect_volume);
        break;
    default:
        break;
    }
